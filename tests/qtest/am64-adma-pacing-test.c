/*
 * QTest for AM64x SDHCI ADMA2 virtual-clock pacing (cmblu/corenode fork)
 *
 * Image-free, deterministic reproducer for the SDHCI ADMA pacing change.
 *
 * The stock QEMU SDHCI model sliced an ADMA2 descriptor chain into batches
 * of SDHC_ADMA_DESCS_PER_DELAY (5) descriptors, rescheduling itself
 * SDHC_TRANSFER_DELAY (100) ns later on QEMU_CLOCK_VIRTUAL between batches.
 * A guest therefore only observed Transfer Complete after several
 * virtual-clock round-trips.  Our fix runs a chain that requests no
 * DMA-boundary interrupt to completion in a single call, so Transfer
 * Complete is asserted synchronously with the command write.
 *
 * This test drives a real CMD18 (READ_MULTIPLE_BLOCK) over a 12-descriptor
 * ADMA2 chain (> DESCS_PER_DELAY) and counts how many QEMU_CLOCK_VIRTUAL
 * steps of SDHC_TRANSFER_DELAY are needed before Transfer Complete latches:
 *
 *   - with the fix (this branch): 0 steps  (complete before any clock step)
 *   - pre-fix (sliced ADMA):     >0 steps  (batched 5 descriptors per call,
 *                                           observed 1 step for this chain)
 *
 * Under "-accel qtest" the virtual clock only advances when the test steps
 * it, which makes the pacing directly observable and the count deterministic.
 *
...
/*
 * SDHCI register offsets, attribute bits and pacing constants, mirrored
 * from hw/sd/sdhci-internal.h.  That header is not standalone-includable
 * from a qtest (its tail declares functions over the device-private
 * SDHCIState type), so -- like the sibling ti-am64-i2c-test.c -- the few
 * definitions this test needs are reproduced here.
 */
...
/* SDHC_ADMA_DESCS_PER_DELAY is 5; NDESC below is chosen larger than it. */

/*
 * The am64-virt machine instantiates two TYPE_SYSBUS_SDHCI controllers
 * (hw/arm/ti-am64x.c): eMMC (MMCSD0) at 0x0fa10000 and SD (MMCSD1) at
 * 0x0fa00000.  A single "-device sd-card" attaches to the first free
 * sd-bus, and which controller that is depends on realize order, so we
 * probe SDHC_PRNSTS.CARD_PRESENT on both bases at runtime and use whichever
 * actually carries the card rather than hard-coding one.
 */
#define SDHCI_SD_BASE     0x0fa00000ULL
#define SDHCI_EMMC_BASE   0x0fa10000ULL

/* Scratch guest DDR (base 0x80000000): ADMA descriptor table + target buf. */
...
/* Bounded poll for command-complete; returns the latched NORINTSTS. */
                       ...
                       uint32_t arg, uint16_t flags)
{
    ...
    /* sdhci_send_command() runs synchronously during the CMDREG write. */
    for (i = 0; i < 1000; i++) {
...
/* Bring the SD card from idle to transfer ("tran") state. */
static void sd_bring_up_card(QTestState *qts, uint64_t base)
{
    ...
    for (int i = 0; i < 100; i++) {
        ...
        /* A non-enquiry ACMD41 powers the model up immediately (single pass);
         * the loop is a bounded safety net, not a busy-wait requirement. */
        break;
    }
...
/*
 * Build a 32-bit ADMA2 descriptor table.  get_adma_description() for
 * SDHC_CTRL_ADMA2_32 reads a little-endian 64-bit word per 8-byte entry:
 *   attr   = bits [6:0]      (VALID | ACT_TRAN [| END])
 *   length = bits [31:16]
 *   addr   = bits [63:32]
 * i.e. word0 = (length << 16) | attr, word1 = target address.  No INT
 * attribute is set, so the fixed model must run the whole chain in one call.
 */
...
static void test_adma_pacing(void)
{
    ...
    /* 1 MiB raw backing file of zeros for the SD card. */
    ...
    /* Select ADMA2 32-bit as the DMA engine (Host Control 1 bits [4:3]). */
    qtest_writeb(qts, base + SDHC_HOSTCTL, SDHC_CTRL_ADMA2_32);

    build_adma_table(qts);

    /* Program the transfer descriptor/geometry and clear stale status. */
    ...
    /* CMD18 READ_MULTIPLE_BLOCK, data present -> kicks off ADMA. */
    qtest_writew(qts, base + SDHC_CMDREG,
                 (18 << 8) | SDHC_CMD_DATA_PRESENT | RESP_R48);

    /*
     * Count SDHC_TRANSFER_DELAY virtual-clock steps until Transfer Complete.
     * Fixed model: already set (0 steps).  Sliced model: one step per extra
     * batch after the first synchronous one (>0 steps for a 12-descriptor
     * chain sliced into batches of SDHC_ADMA_DESCS_PER_DELAY).
     */
    ...
    /* Completion must be a clean Transfer Complete, no error interrupt. */
    ...
    /* The fix completes the whole chain in one pass: no clock steps. */
