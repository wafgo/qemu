...
/* certificate-only image whose components claim the given sizes */
...
{
    /* comp_size INTEGER wider than 32 bits must not silently truncate */
...
{
    /*
     * Each size fits in 32 bits, but the sum wraps a 32-bit size_t;
     * the parser must fail via the running total, never succeed on a
     * wrapped accumulator.
     */
