 ...
 * - 16 entries
 * - 2 GB window starting at 0x6000_0000
 * - Dynamic remap via MemoryRegion alias + container
 *
 * This models the TI RAT as an interconnect feature.
 */
...
static void ti_rat_apply_entry(TIRATState *s, TIRATEntry *e)
{
    ...
    if (size < 0x1000) {
        qemu_log_mask(LOG_GUEST_ERROR, "RAT %u: size too small: 0x%"PRIx64"\n",
                      e->idx, size);
        return;
    }
    ...
    if (source_addr < s->window_base ||
        ...
        qemu_log_mask(LOG_GUEST_ERROR, "RAT %u: source outside window: 0x%"HWADDR_PRIx"\n",
                      e->idx, source_addr);
        return;
    }
    ...
    /* Insert/move subregion exactly once */
    if (!e->inserted) {
        memory_region_add_subregion(&s->window_container, woff, &e->alias);
        e->inserted = true;
    } else {
        /* if woff can change: move it */
        memory_region_del_subregion(&s->window_container, &e->alias);
        memory_region_add_subregion(&s->window_container, woff, &e->alias);
    }
...
static void ti_rat_realize(DeviceState *dev, Error **errp)
{
    ...
    /* register MMIO */
    memory_region_init_io(&s->regs_mmio, OBJECT(s),
                          &ti_rat_ops, s,
                          "ti-rat-regs", RAT_REGS_SIZE);
    sysbus_init_mmio(sbd, &s->regs_mmio);

    memory_region_init(&s->window_container, OBJECT(s),
                       "ti-rat-window",
                       s->window_size);
    memory_region_add_subregion(s->window_root,
                                s->window_base,
                                &s->window_container);

    for (int i = 0; i < TI_RAT_NUM_ENTRIES; i++) {
        g_autofree char *name = g_strdup_printf("ti-rat-alias[%d]", i);
        memory_region_init_alias(&s->ent[i].alias, OBJECT(s),
                                 name,
                                 s->target_root,
                                 0,
                                 0x1000);
...
static const Property ti_rat_props[] = {
    ...
    DEFINE_PROP_UINT64("window-size", TIRATState, window_size,
                       TI_RAT_WINDOW_SIZE),
    DEFINE_PROP_LINK("target-root", TIRATState, target_root,
                     TYPE_MEMORY_REGION, MemoryRegion *),
    DEFINE_PROP_LINK("window-root", TIRATState, window_root,
                 TYPE_MEMORY_REGION, MemoryRegion *),
};
