...
static void ti_sec_proxy_realize(DeviceState *dev_soc, Error **errp)
{
    ...
    for (int i = 0; i < SEC_PROXY_THREAD_ID_MAX; ++i) {
        ...
        ti->thread_id = i;
        //FIXME: Hack, this is set by the dmsc I guess? But we fake it here.
...
size_t ti_sec_proxy_push_msg(TISecProxyState *sp,
                           uint16_t thread_id,
...
static const char *ti_sec_proxy_get_thread_channel_name(enum TISciThreadIds thread_id)
{
    const char *thread_names[] = {
        "MAIN_0_R5_0_READ_RESPONSE_THREAD",
        "MAIN_0_R5_0_WRITE_THREAD",
        "MAIN_0_R5_1_READ_RESPONSE_THREAD",
        "MAIN_0_R5_1_WRITE_THREAD",
        "MAIN_0_R5_2_READ_RESPONSE_THREAD",
        "MAIN_0_R5_2_WRITE_THREAD",
        "MAIN_0_R5_3_READ_RESPONSE_THREAD",
        "MAIN_0_R5_3_WRITE_THREAD",
        "A53_0_READ_RESPONSE_THREAD",
        "A53_0_WRITE_THREAD",
        "A53_1_READ_RESPONSE_THREAD",
        "A53_1_WRITE_THREAD",
        "A53_2_READ_RESPONSE_THREAD",
        "A53_2_WRITE_THREAD",
        "A53_3_READ_RESPONSE_THREAD",
        "A53_3_WRITE_THREAD",
        "M4_0_READ_RESPONSE_THREAD",
        "M4_0_WRITE_THREAD",
        "MAIN_1_R5_0_READ_RESPONSE_THREAD",
        "MAIN_1_R5_0_WRITE_THREAD",
        "MAIN_1_R5_1_READ_RESPONSE_THREAD",
        "MAIN_1_R5_1_WRITE_THREAD",
        "MAIN_1_R5_2_READ_RESPONSE_THREAD",
        "MAIN_1_R5_2_WRITE_THREAD",
        "MAIN_1_R5_3_READ_RESPONSE_THREAD",
        "MAIN_1_R5_3_WRITE_THREAD",
        "A53_4_READ_RESPONSE_THREAD",
        "A53_4_WRITE_THREAD",
        "ICSSG_0_READ_RESPONSE_THREAD",
        "ICSSG_0_WRITE_THREAD",
        "ICSSG_1_READ_RESPONSE_THREAD",
        "ICSSG_1_WRITE_THREAD"
        };
...
static uint64_t ti_sec_proxy_read_rt(void *opaque, hwaddr addr,
                                     unsigned size)
{
    TISecProxyState *s = opaque;
    int thread_num = addr / 0x1000;
                                         ...
                                         unsigned size)
{
    ...
    int byte = off & 0x3;     // byte offset within 32-bit register

    struct TISecProxyThreadInfo *tinfo = &s->thread_info[thread_num];

    /* Fetch the full 32-bit register value first (little-endian device model). */
    ...
    if (size == 1) {
        ...
        /* Optional: enforce halfword alignment. If you prefer strict behavior,
         * you could log+return 0 or trigger a guest error when (byte & 1) != 0.
         * This version allows it by rounding down to the nearest halfword.
         */
                                      ...
                                      unsigned size)
{
    ...
    if (size == 1) {
        ...
        /* Same note as in read: this version allows odd halfword addresses
         * by rounding down to the nearest halfword boundary.
         */
        int hbyte = (byte & ~1);
    ...
    /* Keep your existing semantics: completion when the *last register* is written.
     * Note: with byte writes this can now trigger multiple times if the guest writes
...
static void ti_sec_proxy_init(Object *obj)
{
    ...
    s->reg_array =
        register_init_block32(DEVICE(obj), sec_proxy_mmrs_regs_info,
    ...
    memory_region_init_io(&s->iomem_scfg, OBJECT(s),
                          &ti_sec_proxy_scfg_ops, s,
                          "ti-sec-proxy-scfg", 0x80000);
    sysbus_init_mmio(sbd, &s->iomem_scfg);

    memory_region_init_io(&s->iomem_rt, OBJECT(s),
                          &ti_sec_proxy_rt_ops, s,
