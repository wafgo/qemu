...
/*
 * Minimal TI K3 Global Timebase Counter (GTC) register stub.
 *
 * ATF-BL31 reads the GTC control (CNTCR) and frequency (CNTFID0) registers
 * and refuses to trust the timebase if the counter is disabled or the
 * frequency reads back 0. QEMU does not model the counter itself; this stub
 * reports the counter enabled at a fixed frequency so ATF stops warning
 * "GTC is disabled" / "GTC_CNTFID0 is 0".
 */
